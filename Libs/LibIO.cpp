...
#define METATABLE_FILE "file"
    ...
    int PushError(symvl::StackAPI &api)
    {
        api.PushNil();
    ...
    void ReadBytes(symvl::StackAPI &api, std::FILE *file, int bytes)
    {
        if (bytes <= 0)
            api.PushString("");
        else
    ...
    void ReadByFormat(symvl::StackAPI &api, std::FILE *file,
    ...
    int Close(symvl::State *state)
    {
        symvl::StackAPI api(state);
        if (!api.CheckArgs(1, symvl::ValueT_UserData))
            return 0;
    ...
    int Seek(symvl::State *state)
    {
        symvl::StackAPI api(state);
        if (!api.CheckArgs(1, symvl::ValueT_UserData,
                           symvl::ValueT_String, symvl::ValueT_Number))
            return 0;
    ...
    int Setvbuf(symvl::State *state)
    {
        symvl::StackAPI api(state);
        if (!api.CheckArgs(2, symvl::ValueT_UserData,
                           symvl::ValueT_String, symvl::ValueT_Number))
            return 0;
    ...
    int Write(symvl::State *state)
    {
        ...
        for (int i = 1; i < params; ++i)
        {
            ...
            else if (type == symvl::ValueT_Number)
            {
            ...
            {
                api.ArgTypeError(i, symvl::ValueT_String);
                return 0;
            }
        }
    ...
    int Open(symvl::State *state)
    {
        symvl::StackAPI api(state);
        if (!api.CheckArgs(1, symvl::ValueT_String, symvl::ValueT_String))
            return 0;
    ...
    void RegisterLibIO(symvl::State *state)
    {
        symvl::Library lib(state);
        symvl::TableMemberReg file[] = {
            { "close", Close },
            { "flush", Flush },
            { "read", Read },
            { "seek", Seek },
