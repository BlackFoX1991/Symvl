...
#define MATH_FUNCTION(name, std_name)                       \
    int name(symvl::State *state)                            \
    {                                                       \
        symvl::StackAPI api(state);                          \
        if (!api.CheckArgs(1, symvl::ValueT_Number))         \
            return 0;                                       \
...
#define MATH_FUNCTION2(name, std_name)                      \
    ...
    {                                                       \
        symvl::StackAPI api(state);                          \
        if (!api.CheckArgs(2, symvl::ValueT_Number,          \
                           symvl::ValueT_Number))            \
            return 0;                                       \
        api.PushNumber(std::std_name(api.GetNumber(0),      \
                                     api.GetNumber(1)));    \
    ...
    int Log(symvl::State *state)
    {
        symvl::StackAPI api(state);
        if (!api.CheckArgs(1, symvl::ValueT_Number, symvl::ValueT_Number))
            return 0;
    ...
    int Random(symvl::State *state)
    {
        symvl::StackAPI api(state);
        if (!api.CheckArgs(0, symvl::ValueT_Number, symvl::ValueT_Number))
            return 0;
    ...
    int RandomSeed(symvl::State *state)
    {
        symvl::StackAPI api(state);
        if (!api.CheckArgs(1, symvl::ValueT_Number))
            return 0;
    ...
    void RegisterLibMath(symvl::State *state)
    {
        symvl::Library lib(state);
        symvl::TableMemberReg math[] = {
            { "abs", Abs },
            { "acos", Acos },
            { "asin", Asin },
            { "atan", Atan },
